...
class TestAddCard(unittest.TestCase):
    def test_add_cards(self):
        ...
        collection = []
        test = add_card("036ef8c9-72ac-46ce-af07-83b79d736538", collection, non_collection, "data/test.json", number = 2)
        with open("data/test.json") as f:
            test = f.read()
            self.assertTrue("Storm Crow" in test)
        test = add_card("b125d1e7-5d9b-4997-88b0-71bdfc19c6f2", collection, non_collection, "data/test.json")
        with open("data/test.json") as f:
            test = f.read()
            self.assertTrue("Walking Sponge" in test)
        test = add_card("e0f83824-43c6-4101-88fd-9109958b23e2", collection, non_collection, "data/test.json", number = 0)
        with open("data/test.json") as f:
            test = f.read()
            self.assertFalse("Ravnica at War" in test)
        test = add_card("712fb9e5-bd67-4173-a2d4-061aeb6253b5", collection, non_collection, "data/test.json", number = 1)
        with open("data/test.json") as f:
            test = f.read()
