...
class TestDeckAddCard(unittest.TestCase):
    def test_deck_add_card(self):
        ...
        collection = []
        deck = []
        test = deck_add_card("036ef8c9-72ac-46ce-af07-83b79d736538", deck, collection, commander_legal_cards, "data/decks/test_deck_add_card.json", number = 1)
        with open("data/decks/test_deck_add_card.json") as f:
            test = f.read()
            self.assertTrue('"number_in_deck": 1' in test)
        test = deck_add_card("b125d1e7-5d9b-4997-88b0-71bdfc19c6f2", deck, collection, commander_legal_cards, "data/decks/test_deck_add_card.json")
        with open("data/decks/test_deck_add_card.json") as f:
            test = f.read()
            self.assertTrue("Walking Sponge" in test)
        test = deck_add_card("e0f83824-43c6-4101-88fd-9109958b23e2", deck, collection, commander_legal_cards, "data/decks/test_deck_add_card.json", number = 0)
        with open("data/decks/test_deck_add_card.json") as f:
            test = f.read()
            self.assertFalse("Ravnica at War" in test)
        test = deck_add_card("712fb9e5-bd67-4173-a2d4-061aeb6253b5", deck, collection, commander_legal_cards, "data/decks/test_deck_add_card.json", number = 1)
        with open("data/decks/test_deck_add_card.json") as f:
            test = f.read()
