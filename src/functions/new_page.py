...
def new_page(collection, outer, type, next):
    if outer.count !=0:  
        if next and outer.count >= len(collection):
            return
        elif not next and outer.count <=8:
            return
    ...
    if next:
        ...
        outer.count_label.config(text=f"Displaying {outer.count} - {display_up_to} of {len(collection)}")
        for i in range(outer.count, display_up_to):
            item = collection[i]
            display(type, item, outer.p_frame, i, outer)
        outer.count += (display_up_to - outer.count)
        if outer.count > len(collection):
            outer.count = len(collection)
    else:
        outer.count = outer.count -9
        if outer.count <= 0:
            outer.count = 9
        display_up_to =  outer.count - 9
        if display_up_to < 0:
            display_up_to = 0
        outer.count_label.config(text=f"Displaying {display_up_to} - {outer.count} of {len(collection)}")
        for i in range(display_up_to, outer.count):
            item = collection[i]
            display(type, item, outer.p_frame, i, outer)
    outer.p_frame.grid(sticky="nsew", row=4)
    outer.outer.update_idletasks()



def display(type, item, p_frame, ind, outer):
        frame = None
        if type == "cards":
            frame = card_page(item, p_frame)
        elif type == "decks":
            frame = list_decks_page(item, p_frame, outer)
        x = ind%3
        y = ind//3
        frame.grid(column=x, row=y)
        p_frame.elements.append(frame)
