...
class TestAddDeck(unittest.TestCase):
    def test_add_deck(self):
        ...
        deck_list = [{"name":"Storm Crow", "number":1, "add_to_collection":True}, {"name":"Walking Sponge", "number":1, "add_to_collection":True}, {"name":"Ravnica at War", "number":2, "add_to_collection":True}, {"name":"Urabrask // The Great Work", "number":1, "add_to_collection":True} ]
        collection = []
        name = "test_add_deck"
        test = add_deck(name, collection, commander_legal_cards, "data/test_add_deck.json", deck_list=deck_list)
        with open("data/decks/test_add_deck.json") as f:
            test = f.read()
            self.assertTrue("Storm Crow" in test)
        with open("data/test_add_deck.json") as f:
            test = f.read()
